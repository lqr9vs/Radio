...
local function connecttoradio(channel)
    ...
    if SplitStr(tostring(channel), ".")[2] ~= nil and SplitStr(tostring(channel), ".")[2] ~= "" then
        lib.notify({
            title = 'Radio',
            description = 'You have joined the frequency: ' ..channel.. ' MHz',
            type = 'info'
        })
    else
        lib.notify({
            title = 'Radio',
            description = 'You have joined the frequency: ' ..channel.. '.00 MHz',
            type = 'info'
        })
    end
end
...
local function leaveradio()
    ...
    lib.notify({
        title = 'Radio',
        description = 'You have left the frequency.',
        type = 'info'
    })
end
    ...
    if rchannel ~= nil then
        if rchannel <= Config.MaxFrequency and rchannel ~= 0 then
            if rchannel ~= RadioChannel then
                ...
                lib.notify({
                    title = 'Radio',
                    description = 'You are already connected to this frequency!',
                    type = 'info'
                })
            end
        else
...
RegisterNUICallback('leaveRadio', function(data, cb)
    if RadioChannel == 0 then
        lib.notify({
            title = 'Radio',
            description = 'You are not connected to a frequency.',
            type = 'error'
        })
    else
        leaveradio()
...
RegisterNUICallback("volumeUp", function()
    if RadioVolume <= 95 then
        ...
        lib.notify({
            title = 'Radio',
            description = 'The radio is already at maximum volume.',
            type = 'error'
        })
    end
end)
...
RegisterNUICallback("volumeDown", function()
    if RadioVolume >= 10 then
        ...
        lib.notify({
            title = 'Radio',
            description = 'The radio is already at minimum volume.',
            type = 'error'
        })
    end
end)
...
for i=1, Config.MaxFrequency do
    RegisterNetEvent('esx-radio:client:JoinRadioChannel'.. i, function(channel)
        ...
        if SplitStr(tostring(channel), ".")[2] ~= nil and SplitStr(tostring(i), ".")[2] ~= "" then
            lib.notify({
                title = 'Radio',
                description = 'You have joined the frequency: ' ..i.. ' MHz',
                type = 'info'
            })
        else
            lib.notify({
                title = 'Radio',
                description = 'You have joined the frequency: ' ..i.. '.00 MHz',
                type = 'info'
            })
        end
    end)
...
if Config.KeyMappings.Enabled then
    RegisterKeyMapping("radio", 'Take out the radio', 'keyboard', Config.KeyMappings.Key)
end